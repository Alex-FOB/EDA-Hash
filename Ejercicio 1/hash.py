...
class tablaHash:
    ...
    def __init__(self, claves):
        cant = self.cantNoPrimo(claves//0.7) #self.cantPrimo(claves//0.7) o self.cantNoPrimo(claves//0.7)
        #print('Dimension del arreglo', cant)
    ...
    def cantPrimo(self, cant): #devuelve una cantidad primo
        ...
        if(cant < lista[len(lista)-1]):
            ...
            while not band and i <= 39:
                resultado = (i**2) + i + 41
                #print('Valor de i: {} - Resultado: {}'.format(i, resultado))
                if(resultado > cant):
                    primo = resultado
                    band = True
                i += 1
        return primo
    def cantNoPrimo(self, cant): #devuelvo una cantidad no primo
        dim = int(cant)
        if(self.primo(cant)):
            dim += 1
        return dim
